...
function Grades(){
    ...
    const [students, setStudents] = useState([
      ...
      useEffect(() => {
        ...
        const filteredStudentsWithAssignments = filteredByStudents.map((student) => {
          const filteredAssignments =
            selectedAssignments.length === 0
              ? student.assignments
              : student.assignments.filter((assignment) => selectedAssignments.includes(assignment.name));
      ...
      useEffect(() => {
        ...
        setAssignmentOptions([
          ...new Set(students.flatMap((student) => student.assignments.map((assignment) => assignment.name))),
        ]);
      }, [students]);
...
return(
    ...
    <div>
      ...
      <div className="filter-container">
        ...
        <Select
          isMulti
          options={students.map((student) => ({ value: student.name, label: student.name }))}
          value={students.filter((student) => selectedStudents.includes(student.name)).map((student) => ({ value: student.name, label: student.name }))}
      ...
      <div class="filter-box">
        ...
        <Select
          isMulti
          options={assignmentOptions.map((assignmentName) => ({ value: assignmentName, label: assignmentName }))}
          value={selectedAssignments.map((assignmentName) => ({ value: assignmentName, label: assignmentName }))}
            ...
            <tr key={student.id}>
              ...
              {filteredStudents[0]?.assignments.map((assignment) => (
                <td key={assignment.id}>
                  {student.assignments.find((a) => a.name === assignment.name) ? student.assignments.find((a) => a.name === assignment.name).marks : '-'}
                </td>
              ))}
            </tr>
          ))}
